...
export async function POST(req: Request) {
  ...
  const { user_id, name, domain, country, industry, description, email, phone } = body as Record<string, string>
  ...
  const { data, error } = await client
    .from('companies')
    .insert({ user_id, name, domain, country, industry, description, email, phone })
    .select()
    .single()
